...
for index, row in df.iterrows():
    ...
    # [매수] AI가 '상승(1)'을 예측하고, 그 확신도가 0.7 초과일 때
    if pred_class == 1 and confidence > 0.7:
        # 0.7~1.0 구간을 0.0~1.0 비율로 스케일링
        buy_ratio = (confidence - 0.7) / 0.3
    ...
    # [매도] AI가 '하락(0)'을 예측하고, 그 확신도가 0.7 초과일 때
    elif pred_class == 0 and confidence > 0.7:
        # 0.7~1.0 구간을 0.0~1.0 비율로 스케일링
        sell_ratio = (confidence - 0.7) / 0.3
